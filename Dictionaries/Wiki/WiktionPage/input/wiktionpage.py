...
def find_conjugation(input_string):
    pattern = r'\{\{en-[^\}]*\}\}'
    matches = re.findall(pattern, input_string)
    return matches

wordsfile = os.path.join(os.path.dirname(__file__), "words.txt")
...
conjufile = os.path.join(os.path.dirname(__file__), "conjugations.txt")
...
for word in lines:
    ...
    if found == True:
        ...
        if wiktion_page:
            ...
            if pagetext:
                print(' DOWNLOADED')
                file = codecs.open(conjufile, "a", "utf-8")
                enbrackets = find_conjugation(pagetext)
                for en in enbrackets:
                    file.write(word + ': ' + en + '\n')
                file.close()
            else:
